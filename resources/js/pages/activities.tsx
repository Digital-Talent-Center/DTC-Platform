...
function isOverdue(item: ActivityItem) {
  ...
  // For tasks, check deadline (now includes time)
...
function formatDisplayDateTime(dateStr?: string | null) {
  ...
  // Handle 'YYYY-MM-DD HH:mm' format from API
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSave({
        ...
        deadline: activeType === 'task' ? `${deadline || getTodayStr()} ${deadlineTime || '23:59'}` : undefined,
        status: 'pending',
      });
    } catch (err: any) {
      ...
      <div ref={modalRef} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 pb-4">
          <input type="text" placeholder="Add title" value={title} onChange={(e) => setTitle(e.target.value)}
            className={`w-full text-xl font-medium text-gray-900 placeholder-gray-300 border-b-2 pb-2 focus:outline-none bg-transparent transition-all ${activeType === 'event' ? 'border-amber-400 focus:border-amber-500' : 'border-blue-400 focus:border-blue-500'
              }`} autoFocus />
        </div>
        ...
        <div className="px-6 pb-4 flex items-center gap-2">
          {(['event', 'task'] as ItemType[]).map((t) => (
            <button key={t} onClick={() => setActiveType(t)}
              className={`px-4 py-1.5 text-sm font-medium rounded-full transition-all ${activeType === t
                ? t === 'event'
        ...
        <div className="px-6 pb-6 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <textarea placeholder="Add description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3}
              className={`flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:border-transparent ${activeType === 'event' ? 'focus:ring-amber-400' : 'focus:ring-blue-400'
                }`} />
          </div>
        </div>
        ...
        <div className="px-6 pb-6 flex items-center justify-end gap-3">
          ...
          <button onClick={handleSubmit} disabled={!title.trim() || loading}
            className={`px-6 py-2.5 text-white text-sm font-semibold rounded-xl shadow-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 ${activeType === 'event' ? 'bg-amber-500 hover:bg-amber-600' : 'bg-blue-600 hover:bg-blue-700'
              }`}>
              ...
              <div key={item.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-2.5 py-0.5 text-[10px] font-bold tracking-wider rounded-full ${displayStatus === 'OVERDUE'
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-gray-400">
                    ...
                    {safeDate(item.deadline?.replace(' ', 'T')) && (
                      <span className="flex items-center gap-1.5 text-amber-600">
                        Deadline: {item.deadline ? formatDisplayDateTime(item.deadline) : '-'}
                      </span>
                    )}
                  </div>
                  {/* Action Buttons */}
