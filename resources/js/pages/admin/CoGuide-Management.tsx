...
const emptyForm = { title: '', description: '', category: '', level: '', year: '', tags: '' };
...
export default function CoGuideManagement({ initialGuides, total }: Props) {
    ...
    useEffect(() => { setGuides(initialGuides); }, [initialGuides]);
    ...
    const handleCreate = () => {
        setCreating(true);
        router.post(route('admin.guides.store'), { ...createForm, file }, {
            forceFormData: true,
            preserveScroll: true,
    ...
    const handleView = (guide: Guide) => {
        if (!guide.file_path) { alert('File belum tersedia untuk guide ini.'); return; }
    ...
    const handleDelete = () => {
        if (guideToDelete === null) return;
        router.delete(route('admin.guides.destroy', guideToDelete), {
            preserveScroll: true,
            onSuccess: () => {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 min-h-screen">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                    <div>
                        ...
                        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight">Manajemen Co-Guide</h1>
                    </div>
                        ...
                        <table className="w-full table-fixed text-sm">
                            <thead>
                                <tr className="bg-[#fbbf24]">
                                    <th className="text-left w-[40%] px-8 py-4 text-gray-900 font-bold text-xs uppercase tracking-wider">Judul</th>
                                    <th className="text-left w-[18%] px-8 py-4 text-gray-900 font-bold text-xs uppercase tracking-wider">Level</th>
                                    <th className="text-left w-[24%] px-8 py-4 text-gray-900 font-bold text-xs uppercase tracking-wider">Views / Unduhan</th>
                                ...
                                {visibleGuides.map((guide) => (
                                    <tr key={guide.id} className="hover:bg-gray-50/50 transition-colors">
                                        <td className="px-8 py-4">
                                            <p className="font-bold text-gray-900 line-clamp-1">{guide.title}</p>
                                            <p className="text-xs text-gray-500 mt-0.5">{guide.category || 'Tanpa kategori'}</p>
                                        </td>
                                        <td className="px-8 py-4"><LevelBadge level={guide.level} /></td>
                                        <td className="px-8 py-4 text-gray-500 text-sm">{guide.views_count} views &middot; {guide.downloads_count} unduhan</td>
                                        <td className="px-8 py-4">
                                            <div className="flex items-center gap-4">
                                                <button onClick={() => handleView(guide)} className="text-gray-400 hover:text-gray-700 transition-colors cursor-pointer" title="Lihat File">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                    </svg>
                                                </button>
                                                <button onClick={() => confirmDelete(guide.id)} className="text-gray-400 hover:text-red-600 transition-colors cursor-pointer" title="Hapus">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                ...
                                {guides.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="px-8 py-16 text-center text-gray-500 font-medium">
                                            Belum ada Co-Guide. Klik "Tambah Co-Guide" untuk membuat.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                    ...
                    {guides.length > 0 && (
                        <div className="px-8 py-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <p className="text-sm text-gray-500 font-medium">
                                Menampilkan {Math.min(visibleCount, guides.length)} dari {total} co-guide
                            </p>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-gray-900/40 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl max-w-[34rem] w-full shadow-xl overflow-hidden">
                        <div className="bg-[#7b5b00] px-6 py-4">
                            <h3 className="text-lg font-bold text-white tracking-tight">Tambah Co-Guide</h3>
                        </div>
                        <div className="p-6 grid grid-cols-2 gap-4">
                            <div className="flex flex-col gap-1 col-span-2">
                                ...
                                <input type="text" value={createForm.title} onChange={e => setCreateForm(f => ({ ...f, title: e.target.value }))}
                                    className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder="Judul guide" />
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <input type="text" value={createForm.category} onChange={e => setCreateForm(f => ({ ...f, category: e.target.value }))}
                                    className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder="mis. tutorial" />
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <input type="text" value={createForm.tags} onChange={e => setCreateForm(f => ({ ...f, tags: e.target.value }))}
                                    className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder="algoritma, c++" />
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-gray-900/40 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-xl border border-gray-100">
                        <div className="flex items-start gap-4 mb-5">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-gray-900 tracking-tight">Hapus Co-Guide?</h3>
                        ...
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                            <button onClick={() => { setDeleteModalOpen(false); setGuideToDelete(null); }}
